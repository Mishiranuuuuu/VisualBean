...
public class KineticTextRenderer {
    ...
    private void parseText(String text) {
        ...
        while (m.find()) {
            ...
            String tag = m.group();
            // Process tag
            if (tag.equals("[shake]")) shake = true;
            else if (tag.equals("[/shake]")) shake = false;
            else if (tag.equals("[wave]")) wave = true;
            else if (tag.equals("[/wave]")) wave = false;
            else if (tag.startsWith("[color=")) {
                String val = tag.substring(7, tag.length()-1);
                try {
                    if (val.startsWith("#")) currentColor = Color.decode(val);
                    else if (val.equalsIgnoreCase("red")) currentColor = Color.RED;
                    else if (val.equalsIgnoreCase("blue")) currentColor = Color.BLUE;
                    else if (val.equalsIgnoreCase("green")) currentColor = Color.GREEN;
                    // add more as needed
                } catch (Exception e) {}
            }
            else if (tag.equals("[/color]")) currentColor = null;
